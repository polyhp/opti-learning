...
<div class="w-full flex flex-col space-y-8">
    ...
    <div class="bg-navy-900 rounded-3xl p-8 relative overflow-hidden shadow-xl border border-navy-800">
        ...
        <div class="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
            <div class="text-white">
                ...
                <p class="text-navy-300">Découvrez l'ensemble des formations publiées sur la plateforme.</p>
            </div>
            ...
            <form action="{{ route('formateur.catalog') }}" method="GET" class="w-full md:w-1/2 lg:w-1/3 relative">
                ...
                @if(isset($search) && $search != '')
                    <a href="{{ route('formateur.catalog') }}" class="absolute inset-y-0 right-4 flex items-center text-slate-400 hover:text-white transition-colors">
    ...
    <div class="flex items-center justify-between pb-4 border-b border-slate-200">
        <h2 class="font-head text-xl font-bold text-navy-900 dark:text-white">
            @if(isset($search) && $search != '')
                Résultats pour "{{ $search }}" ({{ $courses->count() }})
            @else
                Toutes les formations ({{ $courses->count() }})
            @endif
        </h2>
        <div class="flex space-x-2">
            <span class="px-4 py-2 bg-slate-100 text-slate-600 rounded-xl text-sm font-medium"><i class="fas fa-filter mr-2"></i>Filtrer</span>
        </div>
    </div>
                ...
                <div class="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm hover:shadow-xl hover:border-orange-200 transition-all duration-300 flex flex-col group relative">
                    ...
                    <div class="p-5 flex-grow flex flex-col justify-between">
                        ...
                        <div class="mt-4 pt-4 border-t border-slate-50">
                            ...
                            <!-- Action View -->
                            <a href="{{ route('courses.show', $course->id) }}" class="mt-4 block w-full text-center py-2.5 bg-slate-50 text-navy-700 font-semibold text-sm rounded-xl hover:bg-navy-900 hover:text-white transition-colors">
    ...
    @else
        <div class="bg-white rounded-3xl border border-slate-100 p-12 text-center shadow-sm">
            ...
            @if(isset($search))
                <a href="{{ route('formateur.catalog') }}" class="inline-block mt-6 px-6 py-3 bg-orange-500 hover:bg-orange-600 text-white font-medium rounded-xl transition-colors">Réinitialiser la recherche</a>
            @endif
        </div>
    @endif
